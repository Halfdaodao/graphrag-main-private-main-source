...
class DRIFTPrimer:
    ...
    async def decompose_query(
        ...
        # DeepSeek supports JSON mode but not the JSON-schema response format
        # generated from a Pydantic model. Parse the JSON response locally.
        model_response: LLMCompletionResponse = await self.chat_model.completion_async(
            messages=prompt, response_format_json_object=True
        )  # type: ignore
        _, parsed_response = try_parse_json_object(model_response.content, verbose=False)
        if not parsed_response:
            parsed_response = {
                "intermediate_answer": model_response.content,
                "score": 0,
                "follow_up_queries": [],
            }
